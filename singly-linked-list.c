...
int insertFirst(headNode* h, int key) {
	...
	node->link = h->first; //node의 링크에 h의 값을 연결하고
	h->first = node; //첫번째에 node 넣어서 바꿈
...
int insertNode(headNode* h, int key) {
	...
	while (temp != NULL) //temp가 널이 아닐동안 반복문
	{
		if (node5->key > temp->key) //node5의 값이 tmep의 값보다 크면
		{
			node6 = temp;
			temp = temp->link;
		}
...
int insertLast(headNode* h, int key) {
	...
	node3->key = key; //node3의 값에 key 넣음
	node2 = h->first;
